    ...
    public static class BankService
    {
        ...
        public static IEnumerable<Account> GetAccounts(int userId)
        {
            ...
            using (SqlConnection conn = Connection)
            {
                conn.Open();
                var cmd = conn.CreateCommand();
                cmd.CommandText = @"Select * from Accounts where userId=@id";
        ...
        public static IEnumerable<Transaction> GetTransactions(int accountId)
        {
            ...
            using (SqlConnection conn = Connection)
            {
                conn.Open();
                var cmd = conn.CreateCommand();
                cmd.CommandText = @"Select * from Transactions where acctid=@id";
        ...
        public static void Transfer(int fromAcct, int toAcct, float amount)
        {
            using (SqlConnection conn = Connection)
            {
                ...
                cmd.CommandText = @"INSERT INTO [dbo].[Transactions] 
                    ...
                    VALUES(@sdate, @acct, @amount,'Transfer',@type)";
                cmd.Parameters.AddWithValue("sdate", sdate);
                cmd.Parameters.AddWithValue("acct", fromAcct);
                cmd.Parameters.AddWithValue("amount", amount);
                cmd.Parameters.AddWithValue("type", 1);
                cmd.ExecuteNonQuery();

                cmd.Parameters["acct"].Value = toAcct;
